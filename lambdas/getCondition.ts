...
export const handler: APIGatewayProxyHandler = async (event, _context) => {
  const illness = event.pathParameters?.illness;
  /*
  if (!illness ||!conditionData[illness]) {
    return apiResponses._400({ message: 'missing ill cond or no data for that illness' });
    }
    */
    return apiResponses._200(conditionData);
  };
  const apiResponses = {
...
const conditionData: IlnessCondition [] = [
      {
